defmodule MangaDl do
  ...
  @num_agents :erlang.size(@user_agents)
  @type error_kind :: :request | :binwrite | :status | :open
  ...
  defp build_page_req(plugin, page_url, chapter, i) do
    Finch.build(
      :get,
      page_url,
      Keyword.merge(
        build_page_headers(),
        plugin.page_headers(chapter, page_url, i)
      ),
      plugin.page_body(chapter, page_url, i)
    )
  end

  defp stream_page({:status, status}, {:ok, file}) do
    if status == 200, do: {:ok, file}, else: {:error, :status, status}
  end

  defp stream_page({:headers, _headers}, {:ok, file}), do: {:ok, file}

  defp stream_page({:data, data}, {:ok, file}) do
    case IO.binwrite(file, data) do
      :ok -> {:ok, file}
      {:error, reason} -> {:error, :binwrite, reason}
    end
  end

  defp stream_page(_, {:error, kind, reason}), do: {:error, kind, reason}

  def download_page({page_url, i}, plugin, chapter, dir) do
    file_name = "#{i + 1}.#{Enum.at(String.split(page_url, "."), -1)}"
    file_dir = "#{dir}/#{file_name}"

    case File.open(file_dir, [:write, :raw]) do
      {:ok, file} ->
        case build_page_req(plugin, page_url, chapter, i)
             |> Client.stream({:ok, file}, &stream_page/2) do
          {:ok, _} ->
            File.close(file)
            :ok
